...
FILE_HEADER = """
/* File generated by gen_cfile_no_canfestival.py. Should not be modified. */
"""
...
"""
    ...
    ctx["file_include_name"] = headerfile.replace(".", "_").upper()
    globalsHeader = ctx.text(FILE_HEADER)
    globalsHeader %= """
#ifndef {file_include_name}
#define {file_include_name}
...
{{
"""
    globalsHeader += headerFileContent
    globalsHeader %= "}}\n#endif // {file_include_name}\n"

    return str(fileContent), str(header), str(globalsFileContent), str(globalsHeader)
...
def GenerateFile(filepath: TPath, node: NodeProtocol, pointers_dict=None):
    ...
    globalsHeaderPath = constantsPath.with_suffix(".h")
    content, header, constantsContent, globalsHeader = generate_file_content(
    ...
    with open(globalsHeaderPath, "w", encoding="utf-8") as f:
        f.write(globalsHeader)
